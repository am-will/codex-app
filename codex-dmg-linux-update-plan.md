...
- **status**: Completed
- **RED evidence**:
  - `npm test -- --runInBand tests/linux/linux-regression.contract.test.ts` from `desktop` -> failed as expected with missing `src/main/linux/protocol-registration.ts` for protocol desktop entry, install plan, and Forge desktop-entry contract tests.
- **GREEN validation**:
  - `npm test -- --runInBand tests/linux/linux-regression.contract.test.ts` from `desktop` -> `1 passed`, `10 passed`, `3 skipped`.
  - `npm test -- --runInBand tests/linux/t5-platform-abstraction.red.test.ts tests/linux/linux-regression.contract.test.ts` from `desktop` -> `2 passed`, `13 passed`, `3 skipped`.
  - `npm test -- --runInBand tests/linux/codex-package.red.test.ts` from `desktop` -> `1 passed`, `6 passed`.
  - `npm test -- --runInBand tests/linux/linux-regression.contract.test.ts tests/linux/t5-platform-abstraction.red.test.ts tests/linux/codex-package.red.test.ts` from `desktop` -> `3 passed`, `19 passed`, `3 skipped`.
  - `desktop-file-validate assets/linux/codex-appimage.desktop` from `desktop` -> passed with no output.
  - `git diff --check` -> no whitespace errors.
- **log**: 2026-04-23: Added a canonical Linux protocol registration source module for `codex`, `x-scheme-handler/codex`, `codex-desktop.desktop`, `%u` URL delivery, desktop entry rendering, and install command planning (`desktop-file-validate`, `update-desktop-database`, `xdg-mime default`). Wired Linux platform capabilities to expose protocol registration separately from autostart, kept cold-start and second-instance callback parsing covered, and configured Forge deb/AppImage desktop generation to ship `MimeType=x-scheme-handler/codex;` plus `%u`.
- **files edited/created**: `desktop/src/main/linux/protocol-registration.ts`, `desktop/src/main/linux/platform-capabilities.ts`, `desktop/src/main/platform-capability-types.ts`, `desktop/src/main/platform-capabilities.ts`, `desktop/forge.config.ts`, `desktop/assets/linux/codex-deb.desktop.ejs`, `desktop/assets/linux/codex-appimage.desktop`, `desktop/tests/linux/linux-regression.contract.test.ts`, `codex-dmg-linux-update-plan.md`
...
- **status**: Completed
- **log**: 2026-04-23: Installed the staged canary to `/home/amwill/.local/opt/codex-desktop/26.417.41555-1858`, updated `/home/amwill/.local/opt/codex-desktop/current`, preserved `/home/amwill/.local/bin/codex-desktop` as the stable wrapper, rewrote the launcher to `Exec=/home/amwill/.local/bin/codex-desktop %u` plus `MimeType=x-scheme-handler/codex;`, rewrote autostart to `Exec=/home/amwill/.local/bin/codex-desktop --open-at-login`, refreshed the desktop database, and set `xdg-mime default codex-desktop.desktop x-scheme-handler/codex`. Validation: `desktop-file-validate /home/amwill/.local/share/applications/codex-desktop.desktop`; `xdg-mime query default x-scheme-handler/codex` -> `codex-desktop.desktop`; `timeout 8s xdg-open 'codex://connector/oauth_callback?code=xdg-test&state=xdg-state'` -> exit `0`; direct isolated cold-start and second-instance protocol tests against `/home/amwill/.local/opt/codex-desktop/current/Codex` reached `window ready-to-show`, initialized `codex-cli 0.121.0`, and invoked the app OAuth callback path, which failed only with expected synthetic-state backend error `OAuth state not found`. Caveat: the real live user-data profile is still held by the old running `26.415.20818-undo-fix-v8` process until Codex is quit/reopened, so same-profile `xdg-open` dispatch will be fully on the new binary after restart.
- **files edited/created**: `/home/amwill/.local/opt/codex-desktop/26.417.41555-1858`; `/home/amwill/.local/opt/codex-desktop/current`; `/home/amwill/.local/bin/codex-desktop`; `/home/amwill/.local/share/applications/codex-desktop.desktop`; `/home/amwill/.config/autostart/codex-desktop.desktop`; `/tmp/codex-live-protocol-userdata-D9v9IU`; `/tmp/codex-live-protocol-cold-C4Fmq5.log`; `/tmp/codex-live-protocol-second-blCfhs.log`
