...
describe('Automation run archive regression gate (RED)', () => {
  test('worktree automations derive starting state from the active branch and fall back to HEAD', () => {
    ...
    expect(mainSource).toMatch(
      /[A-Za-z_$][\w$]*=[A-Za-z_$][\w$]*\.executionEnvironment===`worktree`&&![A-Za-z_$][\w$]*;if\([A-Za-z_$][\w$]*&&[A-Za-z_$][\w$]*==null\)throw Error\(`Worktree automations require a project with (?:one |a )folder`\);let [A-Za-z_$][\w$]*=[A-Za-z_$][\w$]*&&\(await [A-Za-z_$][\w$]*\.getWorktreeRepository\([A-Za-z_$][\w$]*,[A-Za-z_$][\w$]*\)\)\?\.root!=null/,
    );
    expect(mainSource).toMatch(
